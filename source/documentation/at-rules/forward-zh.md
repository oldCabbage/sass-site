...
title: "@forward"
introduction: >
  The `@forward` rule 加载一个 Sass 样式表，并在其被 [`@use` rule](/documentation/at-rules/use) 加载时使它的
  [mixins](/documentation/at-rules/mixin),
  [functions](/documentation/at-rules/function), 和
  [variables](/documentation/variables) 可用。它使得可以将 Sass 库组织在多个文件中，同时允许用户仅加载一个入口文件。
---
